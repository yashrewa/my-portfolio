...
import yash from '../../../public/logo_new_new_new.png';
    ...
    return (
        <>
            ...
            <nav className="desktop-nav hidden md bg-[#141414] z-20 border-r borderteal-400 w-48 h-full fixed top-0 left-0 md:flex flex-col justify-center items-end gap-y-4 ">
                <span className={`w-40 absolute top-2 link desktop ${path === "/" ? 'border-r-4 border-r-primary-purple' : 'border-r-4 border-[#141414]'}`} id='home'> <Link href="/"><Image src={yash} className='mix-blend-screen p-2' alt='logo' width={140} height={140} ></Image></Link></span>
            ...
            <AnimatePresence>
                {navbar &&
                    <motion.div variants={{
                        ...
                        className={`border-right absolute index-high z-50 bg-[rgb(17,17,17)] w-48 top-0 left-0 gap-y-4 flex-col h-screen items-end nav mobile-nav`}>
                        <motion.div className="w-40 mt-4 absolute index-high z-50 top-2 link mobile"><span className="logo mix-blend-screen"><Link href="/"><Image src={yash} className='index-low absolute' alt='logo' width={60} height={60} ></Image></Link></span><span onClick={() => toggleNavbar()} className='index-low absolute top-2 right-8 cursor-pointer'><AiOutlineClose className='text-2xl' /></span></motion.div>
